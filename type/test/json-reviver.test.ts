...
import { _, type Dict } from '../src/mod.ts';

describe('jsonDeserialize reviver', () => {
  describe('autoRegExp – basic patterns', () => {
    it('converts { pattern } to RegExp', () => {
      const result = _.deserialize<Dict>('{"re":{"pattern":"abc"}}', { autoRegExp: true });
    ...
    it('converts { pattern, flags } to RegExp', () => {
      const result = _.deserialize<Dict>('{"re":{"pattern":"abc","flags":"gi"}}', { autoRegExp: true });
      expect(result.re).toBeInstanceOf(RegExp);
      expect((result.re as RegExp).source).toBe('abc');
    ...
    it('converts { regex } to RegExp', () => {
      const result = _.deserialize<Dict>('{"re":{"regex":"abc"}}', { autoRegExp: true });
      expect(result.re).toBeInstanceOf(RegExp);
      expect((result.re as RegExp).source).toBe('abc');
      expect((result.re as RegExp).flags).toBe('');
    });
    ...
    it('converts { regex, flags } to RegExp', () => {
      const result = _.deserialize<Dict>('{"re":{"regex":"abc","flags":"m"}}', { autoRegExp: true });
      expect(result.re).toBeInstanceOf(RegExp);
      expect((result.re as RegExp).source).toBe('abc');
    ...
    it('converts with multiple flags', () => {
      const result = _.deserialize<Dict>('{"re":{"pattern":"test","flags":"gimsuy"}}', { autoRegExp: true });
      expect(result.re).toBeInstanceOf(RegExp);
  ...
  describe('autoRegExp – special regex patterns', () => {
    it('converts pattern with anchors', () => {
      const result = _.deserialize<Dict>('{"re":{"pattern":"^test$","flags":"m"}}', { autoRegExp: true });
      expect(result.re).toBeInstanceOf(RegExp);
    ...
    it('converts pattern with quantifiers', () => {
      const result = _.deserialize<Dict>('{"re":{"pattern":"\\\\d+","flags":""}}', { autoRegExp: true });
      expect(result.re).toBeInstanceOf(RegExp);
    ...
    it('converts pattern with alternation', () => {
      const result = _.deserialize<Dict>('{"re":{"pattern":"cat|dog"}}', { autoRegExp: true });
      expect(result.re).toBeInstanceOf(RegExp);
    ...
    it('converts pattern with character classes', () => {
      const result = _.deserialize<Dict>('{"re":{"pattern":"[a-z]"}}', { autoRegExp: true });
      expect(result.re).toBeInstanceOf(RegExp);
  ...
  describe('autoRegExp – edge cases', () => {
    it('converts empty pattern string to empty RegExp', () => {
      const result = _.deserialize<Dict>('{"re":{"pattern":""}}', { autoRegExp: true });
      expect(result.re).toBeInstanceOf(RegExp);
    });
  ...
  describe('autoRegExp – invalid patterns (returns original value)', () => {
    it('returns original object for unterminated character class', () => {
      const result = _.deserialize<Dict>('{"re":{"pattern":"[invalid"}}', { autoRegExp: true });
    ...
    it('returns original object for invalid flags', () => {
      const result = _.deserialize<Dict>('{"re":{"pattern":"abc","flags":"zz"}}', { autoRegExp: true });
      expect(result.re).not.toBeInstanceOf(RegExp);
  ...
  describe('autoRegExp – disabled', () => {
    it('does not convert when autoRegExp is false', () => {
      const result = _.deserialize<Dict>('{"re":{"pattern":"abc"}}', { autoRegExp: false });
      expect(result.re).not.toBeInstanceOf(RegExp);
      expect(result.re).toEqual({ pattern: 'abc' });
    });

    it('does not convert when autoRegExp is not set (default falsy)', () => {
      const result = _.deserialize<Dict>('{"re":{"pattern":"abc"}}');
      expect(result.re).not.toBeInstanceOf(RegExp);
      expect(result.re).toEqual({ pattern: 'abc' });
    });
  });
  ...
  describe('autoRegExp – nested structures', () => {
    it('converts RegExpDef nested inside an object', () => {
      const result = _.deserialize<{ outer: Dict }>('{"outer":{"inner":{"pattern":"hello","flags":"i"}}}', {
        autoRegExp: true,
      });
    ...
    it('converts RegExpDef inside an array', () => {
      const result = _.deserialize<{ list: unknown[] }>('{"list":[{"pattern":"a"},{"pattern":"b","flags":"i"}]}', {
        autoRegExp: true,
      });
    ...
    it('converts RegExpDef at root level when JSON is a RegExpDef', () => {
      const result = _.deserialize('{"pattern":"abc","flags":"g"}', { autoRegExp: true });
  ...
  describe('autoRegExp – interaction with other features', () => {
    it('autoRegExp and autoTemporal both work independently', () => {
      ...
      const result = _.deserialize<Dict>(json, { autoRegExp: true, autoTemporal: true });
      expect(result.re).toBeInstanceOf(RegExp);
      expect((result.re as RegExp).source).toBe('abc');
    ...
    it('autoRegExp and replace both work independently', () => {
      ...
      const result = _.deserialize<Dict>(json, {
        autoRegExp: true,
        replace: { name: 'World' },
        pre: '${',
        post: '}',
    ...
    it('__filter RegExp takes precedence over autoRegExp', () => {
      ...
      const result = _.deserialize<Dict>(json, { decode: true, autoRegExp: true });
      expect(result.re).toBeInstanceOf(RegExp);
  ...
  describe('__filter RegExp edge cases', () => {
    it('decodes valid __filter RegExp', () => {
      const result = _.deserialize<Dict>('{"re":{"__filter":"RegExp","regex":"abc","flags":"gi"}}', {
        decode: true,
      });
      expect(result.re).toBeInstanceOf(RegExp);
      expect((result.re as RegExp).source).toBe('abc');
    ...
    it('returns original value for invalid RegExp in __filter', () => {
      const result = _.deserialize<Dict>('{"re":{"__filter":"RegExp","regex":"[invalid"}}', { decode: true });
      expect(result.re).not.toBeInstanceOf(RegExp);
    ...
    it('returns original value for __filter RegExp with missing regex key', () => {
      const result = _.deserialize<Dict>('{"re":{"__filter":"RegExp"}}', { decode: true });
      expect(result.re).not.toBeInstanceOf(RegExp);
      expect(result.re).toEqual({ __filter: 'RegExp' });
    });

    it('does not decode __filter RegExp when decode is false', () => {
      const result = _.deserialize<Dict>('{"re":{"__filter":"RegExp","regex":"abc"}}', { decode: false });
      expect(result.re).not.toBeInstanceOf(RegExp);
