...
describe('date-util', () => {
  ...
  describe('tz set deprecated offset', () => {
    it('should set and get timezone offsets', () => {
      ...
      expect(d.getTzOffset()).toEqual(-300);
      // Test with ISOTZ string
      d.setTz('-05:00' as ISOTZ);
      expect(d.getTzOffset()).toEqual(-300);
    });
  });
  describe('tz deprecated half-hour offsets', () => {
    it('should handle Newfoundland (America/St_Johns) UTC-3:30', () => {
      ...
      expect(d.getTzOffset()).toEqual(-210);
    ...
    it('should handle Adelaide (Australia/Adelaide) with half-hour offset', () => {
      ...
      expect(d.getTzOffset()).toEqual(570);
  ...
  describe('withTz', () => {
    it('local', () => {
      Deno.env.set('TZ', 'America/Costa_Rica');
      const d: DateTime = DateTime.fromComponents(2024, 1, 1, 11, 59, 59, 456).withTz();
  ...
  describe('toISOLocaleString', () => {
    ...
    it('default', () => {
      ...
      expect(DateTime.fromDate(d).toISOLocalString()).toEqual('1997-11-25T06:13:14.456-06:00');
    });
    it('show milliseconds', () => {
      expect(DateTime.fromDate(d).toISOLocalString(true)).toEqual('1997-11-25T06:13:14.456-06:00');
    });
    it('hide milliseconds', () => {
      expect(DateTime.fromDate(d).toISOLocalString(false)).toEqual('1997-11-25T06:13:14-06:00');
    });
  });
  ...
  describe('toISOLocaleString tz 0', () => {
    ...
    it('default', () => {
      expect(d.toISOString()).toEqual('1997-11-25T12:13:14.456Z');
      expect(du.toISOLocalString()).toEqual('1997-11-25T12:13:14.456+00:00');
    });
    it('show milliseconds', () => {
      expect(du.toISOLocalString(true)).toEqual('1997-11-25T12:13:14.456+00:00');
    });
    it('hide milliseconds', () => {
      expect(du.toISOLocalString(false)).toEqual('1997-11-25T12:13:14+00:00');
    });
  });
  it('formatLocale', () => {
    const d = new Date('1997-11-25T12:13:14.456Z');
    expect(DateTime.fromDate(d).format('yyyy-MM-dd')).toEqual('1997-11-25');
    expect(DateTime.fromDate(d).format('yyyyMMdd')).toEqual('19971125');
    expect(DateTime.fromDate(d).format('yyyyMMdd_HHmmss')).toEqual('19971125_061314');
    expect(DateTime.fromDate(d).formatUTC('yyyyMMdd_HHmmss')).toEqual('19971125_121314');
  });
  describe('format with month names', () => {
    const d = new Date('2024-01-15T12:30:45.123Z');
    it('MMMM (full month name)', () => {
      expect(DateTime.fromDate(d).formatUTC('MMMM yyyy')).toEqual('January 2024');
    });
    it('MMM (abbreviated month name)', () => {
      expect(DateTime.fromDate(d).formatUTC('MMM dd, yyyy')).toEqual('Jan 15, 2024');
    });
    it('MM (zero-padded month number)', () => {
      expect(DateTime.fromDate(d).formatUTC('yyyy-MM-dd')).toEqual('2024-01-15');
    });
    it('M (month number without padding)', () => {
      expect(DateTime.fromDate(d).formatUTC('M/dd/yyyy')).toEqual('1/15/2024');
    });
    it('combined format', () => {
      expect(DateTime.fromDate(d).formatUTC('dd MMM yyyy HH:mm:ss')).toEqual('15 Jan 2024 12:30:45');
    });
    it('full format with milliseconds', () => {
      expect(DateTime.fromDate(d).formatUTC('MMMM dd, yyyy HH:mm:ss.SSS')).toEqual('January 15, 2024 12:30:45.123');
    });
    it('December test', () => {
      const dDec = new Date('2024-12-25T00:00:00Z');
      expect(DateTime.fromDate(dDec).formatUTC('MMMM dd, yyyy')).toEqual('December 25, 2024');
      expect(DateTime.fromDate(dDec).formatUTC('MMM dd')).toEqual('Dec 25');
    });
  });
  describe('format with unpadded day and hour', () => {
    it('d (day without padding)', () => {
      const d1 = new Date('2024-01-05T00:00:00Z');
      expect(DateTime.fromDate(d1).formatUTC('M/d/yyyy')).toEqual('1/5/2024');
      const d15 = new Date('2024-01-15T00:00:00Z');
      expect(DateTime.fromDate(d15).formatUTC('M/d/yyyy')).toEqual('1/15/2024');
    });
    it('dd (day with padding)', () => {
      const d = new Date('2024-01-05T00:00:00Z');
      expect(DateTime.fromDate(d).formatUTC('MM/dd/yyyy')).toEqual('01/05/2024');
    });
    it('H (hour without padding)', () => {
      const d1 = new Date('2024-01-15T03:30:45Z');
      expect(DateTime.fromDate(d1).formatUTC('H:mm:ss')).toEqual('3:30:45');
      const d12 = new Date('2024-01-15T12:30:45Z');
      expect(DateTime.fromDate(d12).formatUTC('H:mm:ss')).toEqual('12:30:45');
      const d0 = new Date('2024-01-15T00:30:45Z');
      expect(DateTime.fromDate(d0).formatUTC('H:mm:ss')).toEqual('0:30:45');
    });
    it('HH (hour with padding)', () => {
      const d = new Date('2024-01-15T03:30:45Z');
      expect(DateTime.fromDate(d).formatUTC('HH:mm:ss')).toEqual('03:30:45');
    });
    it('combined d and H', () => {
      const d = new Date('2024-01-05T03:30:45Z');
      expect(DateTime.fromDate(d).formatUTC('M/d/yyyy H:mm:ss')).toEqual('1/5/2024 3:30:45');
    });
  });
  describe('format with weekday names', () => {
    const d = new Date('2024-01-15T12:30:45.123Z'); // This is a Monday
    it('EEEE (full weekday name)', () => {
      expect(DateTime.fromDate(d).formatUTC('EEEE, MMMM dd, yyyy')).toEqual('Monday, January 15, 2024');
    });
    it('EEE (abbreviated weekday name)', () => {
      expect(DateTime.fromDate(d).formatUTC('EEE, MMM dd, yyyy')).toEqual('Mon, Jan 15, 2024');
    });
  ...
  it('julianDate', () => {
    const d = new Date('1997-11-25T12:13:14.456Z');
    expect(DateTime.fromDate(d).julianDate()).toBeCloseTo(2450778.0091950926, 6);
    const d1 = new Date('2024-12-30T12:00:00Z');
    expect(DateTime.fromDate(d1).julianDate()).toEqual(2460675);
  });
  describe('googleSheetsDate', () => {
    ...
    it('should produce the correct serial number for a specific timezone', () => {
      ...
      expect(serial).toBeCloseTo(45292.291666666664, 10);
    });
  });
  ...
  describe('ianaTzParse', () => {
    it('should parse America/New_York EDT', () => {
      ...
      expect(offset0).toBe(-300);
    });

    it('should parse America/New_York EST', () => {
  ...
  describe('fromPdfDate', () => {
    it.skip('CST1', () => {
      ...
      if (d instanceof DateTime) {
        expect(d.toISOString()).toBe('2024-01-01T18:00:00.000Z');
      }
    });
    it.skip('CST2', () => {
      Deno.env.set('TZ', 'CST');
    ...
    it('AST +03:00', () => {
      ...
      expect(d).toBeDefined();
      if (d instanceof DateTime) {
        expect(d.toISOString()).toBe('2024-01-01T12:00:00+00:00');
  ...
  describe('DateTime.now()', () => {
    ...
    it('should return an Instant (can add timezone with withTz)', () => {
      ...
      expect(ny.getTzOffset()).toBeDefined();
    });
  });
  ...
  describe('comparison methods', () => {
    describe('isValid', () => {
      ...
      it('should return false for invalid values', () => {
        ...
        expect(DateTime.isValid(null)).toBe(true);
        expect(DateTime.isValid(undefined)).toBe(true);
    ...
    describe('equals', () => {
      ...
      it('should throw for PlainDateTime', () => {
        const d1 = DateTime.fromComponents(2024, 2, 15, 10, 30); // PlainDateTime
        const d2 = DateTime.from('2024-03-15T10:30:00Z');
  ...
  describe('isNow()', () => {
    ...
    it('should work with larger tolerances (days)', () => {
      const yesterday = DateTime.from(DateTime.now().valueOf() - 24 * 60 * 60 * 1000); // 1 day ago
  ...
  describe('add', () => {
    ...
    it('should work with PlainDateTime', () => {
      Deno.env.set('TZ', 'America/Costa_Rica');
      const d = DateTime.fromComponents(2024, 2, 15, 10, 30); // March 15, 2024 10:30
      const future = d.add({ days: 1 });
      expect(future.toString({ timeZoneName: 'never' })).toEqual('2024-02-16T10:30:00-06:00');
    });
  });
  ...
  describe('subtract', () => {
    ...
    it('should work with PlainDateTime', () => {
      ...
      const d = DateTime.fromComponents(2024, 2, 15, 10, 30); // March 15, 2024 10:30
      const past = d.subtract({ days: 1 });
      expect(past.toString({ timeZoneName: 'never' })).toEqual('2024-02-14T10:30:00-06:00');
    });
  ...
  describe('with()', () => {
    ...
    it('should replace components on PlainDateTime', () => {
      ...
      const floored = d.with({ minute: 0, second: 0, millisecond: 0 });
      expect(floored.toString()).toEqual('2024-02-15T10:00:00');
    });
