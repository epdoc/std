...
`@epdoc/std` is a **Deno monorepo** containing seven independently published utility packages. The root `deno.json` defines the workspace members. Each package is published to JSR under the `@epdoc` scope.
...
> Note: `@epdoc/string` and `@epdoc/response` place their entry point at the workspace root (`mod.ts`), not under `src/`.
...
- `isString`, `isNumber`, `isBoolean`, `isDate`, etc. — type guard functions
...
@epdoc/fs         →  (no internal @epdoc deps; targets Deno/Node/Bun)
...
```bash
deno task test          # Run all workspace tests
...
deno update --latest    # Update dependencies after publishing a dependency
bump -g "message"       # Bump version, commit, and push (uses @epdoc/bump)
bump -g -c "message"    # Also update CHANGELOG.md
bump -g -c -t "message" # Also create and push a git tag
```
...
`@epdoc/fs` is actively being modified to support Node.js and Bun in addition to Deno. When editing this package, be aware that Deno-specific APIs may need Node.js equivalents.

### Version Bumping Workflow

Since these are published packages, always use the `bump` tool rather than manually editing version fields:
...
The root `deno.json` imports reflect the **published JSR versions** of workspace packages, used for cross-workspace testing. When developing locally, workspaces reference each other through the Deno workspace protocol (resolved automatically).
