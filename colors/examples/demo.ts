...
import * as Color from 'jsr:@epdoc/colors@^1.1.3';

const W1 = 10;
const W2 = 20;

console.log('\n--- @epdoc/colors Color Palettes ---');
console.log(
  ' ' + 'Dim'.padEnd(W1) + ' | ' + 'Foreground'.padEnd(W1) + ' | ' + 'Bold'.padEnd(W1) +
    ' | ' + 'White on Background'.padEnd(W2) + ' | ' + 'Black on Background'.padEnd(W2) + ' | ' +
    'Hex Value'.padEnd(W1) + ' | ' + 'RGB',
);
...
for (const [name, hex] of Object.entries(Color.palette)) {
  ...
  const rgbStr = [(hex >> 16) & 0xff, (hex >> 8) & 0xff, hex & 0xff].join(',');
  ...
  console.log(
    ` ${styledDim} | ${styled1} | ${styledBold} | ${styled4} | ${styled5} | ${hexStr.padEnd(W1)} | ${rgbStr}`,
  );
}
console.log(''.padEnd(120, '-'));
console.log('\n');
