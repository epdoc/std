...
The @epdoc/transform package is a TypeScript utility library that provides
utilities for\
deep copying, JSON serialization/deserialization, and string replacement
operations.

Key features:

- **Deep copying** with optional string replacement and transformation
- **JSON serialization/deserialization** preserving special types (Set, Map,
  RegExp, Uint8Array)
...
- **Simple replacement**: Use `Record<string, string>` for basic
  string-to-string replacement
- **Advanced replacement**: Use `Record<string, unknown>` with `msubFn` for date
  formatting and complex transformations
...
- `jsonSerialize(value, options?, space?)` - Serialize with special type
  preservation
...
For advanced string replacement features like date formatting, install
`@epdoc/string`:
...
[sindresorhus/strip-json-comments](https://github.com/sindresorhus/strip-json-comments/blob/main/readme.md)
which is\
